...
description: Containerlab engineering rules for writing, reviewing, or refactoring Go code, CLI commands, topology parsing, schemas, docs, nodes, links, endpoints, runtimes, deploy/destroy/apply/reconcile lifecycle flows, and tests. Use for any Containerlab change where user compatibility, operational safety, package boundaries, extension points, context/error/logging behavior, or docs/schema/test coverage matters.
---
...
| 5 | Link, endpoint, node, and runtime contracts | HIGH | `contracts-` |
...
- `contracts-links-endpoints-nodes-runtimes` - Put link behavior on links, endpoint behavior on endpoints, kind behavior on nodes, and provider behavior on runtimes.
...
3. Before adding conditionals for a kind, link type, endpoint type, runtime, or command mode, search for an existing interface, registry, parser, or options structure.
...
- Is link, endpoint, node, or runtime behavior implemented at the owning abstraction rather than with broad type switches or kind checks?
