...
type ContainerRuntime interface {
	...
	WithMgmtNet(*containerlabtypes.MgmtNet)
	...
	PullImage(context.Context, string, containerlabtypes.PullPolicyValue) error
	// CreateContainer creates a container, but does not start it
	CreateContainer(context.Context, *containerlabtypes.NodeConfig) (string, error)
	...
	ListContainers(context.Context, []*containerlabtypes.GenericFilter) ([]GenericContainer, error)
	...
	Exec(ctx context.Context, cID string, execCmd *containerlabexec.ExecCmd) (*containerlabexec.ExecResult, error)
	// ExecNotWait executes cmd on container identified with id but doesn't wait for output nor attaches stdout/err
	ExecNotWait(ctx context.Context, cID string, execCmd *containerlabexec.ExecCmd) error
	...
	GetCooCBindMounts() containerlabtypes.Binds
}
...
type RuntimeConfig struct {
	...
	Debug            bool
	KeepMgmtNet      bool
	VerifyLinkParams *containerlablinks.VerifyLinkParams
}
...
func WithMgmtNet(mgmt *containerlabtypes.MgmtNet) RuntimeOption {
	return func(r ContainerRuntime) {
		r.WithMgmtNet(mgmt)
	}
}
