...
With this approach, you could bridge VM-based routing systems by attaching interfaces to the bridge you define in your topology, however it doesn't allow users to define the VM based nodes in the same topology file. With [`vrnetlab`](https://github.com/plajjan/vrnetlab) integration, containerlab is now capable of launching topologies with VM-based routers defined in the same topology file.
