...
A warm welcome to the [`f5_bigip-ve`](../manual/kinds/f5_bigipve.md) kind! With this addition, Containerlab users can now spin up F5 BIG-IP Virtual Edition appliances as part of their network labs. Built on top of [vrnetlab](../manual/vrnetlab.md), the BIG-IP VE node features management passthrough, day-0 automation for initial configuration and password setup, and configurable resource sizing via environment variables.
...
Why did we need it? To make [Containerlab VS Code extension](../manual/containerlab-gui/vsc-extension.md) real time without polling!
...
Lab and node names are now validated against regex at topology parse time. This catches invalid characters and naming issues early -- before the lab deployment even starts -- and warns against DNS incompatible characters, providing clearer error messages and preventing hard-to-debug failures down the line.

Thanks @juliusliu02 #3018
