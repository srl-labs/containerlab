...
Verify link eth1 in keysight_ixia-c-one node n1
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo containerlab --runtime ${runtime} exec -t ${CURDIR}/${lab-file-name} --label clab-node-name\=n1 --cmd "ip link show ${ifc1-name}"
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
...
Verify link eth2 in keysight_ixia-c-one node n1
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo containerlab --runtime ${runtime} exec -t ${CURDIR}/${lab-file-name} --label clab-node-name\=n1 --cmd "ip link show ${ifc2-name}"
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
    Should Contain    ${output}    state UP
