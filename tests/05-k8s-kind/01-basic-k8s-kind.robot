...
Verify link ${if1-name} in k8s-kind node k01-control-plane
    ...
    ...    sudo -E docker exec -t k01-control-plane ip --color=never address show dev ${if1-name}
    Log    ${output}
...
Verify link ${if1-name} in k8s-kind node k01-worker
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo -E docker exec -t k01-worker ip --color=never address show dev ${if1-name}
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
...
Verify link ${if1-name} in k8s-kind node k02-control-plane
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo -E docker exec -t k02-control-plane ip --color=never address show dev ${if1-name}
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
...
Verify link eth2 in k8s-kind node k01-control-plane
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo -E docker exec -t k01-control-plane ip --color=never link show dev eth2
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
...
Verify link eth2 in k8s-kind node k02-control-plane
    ${rc}    ${output} =    Run And Return Rc And Output
    ...    sudo -E docker exec -t k02-control-plane ip --color=never link show dev eth2
    Log    ${output}
    Should Be Equal As Integers    ${rc}    0
    Should Contain    ${output}    state UP
