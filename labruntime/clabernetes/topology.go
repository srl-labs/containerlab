...
// images. An empty name leaves imagePull unset entirely: the controller fails closed on a named
// Secret that does not exist, so a Topology must only reference one the deploy actually asked
// for.
