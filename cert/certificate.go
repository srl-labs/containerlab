...
func NewCertificateFromFile(certFilePath, keyFilePath, csrFilePath string) (*Certificate, error) {
	...
	cert.Cert, err = containerlabutils.ReadFileContent(certFilePath)
	if err != nil {
		return nil, err
	}
	...
	cert.Key, err = containerlabutils.ReadFileContent(keyFilePath)
	if err != nil {
		return nil, err
	}
	...
	if csrFilePath != "" {
		...
		if err != nil {
			...
			cert.Csr, err = containerlabutils.ReadFileContent(csrFilePath)
			if err != nil {
				return nil, err
			}
		}
...
func (c *Certificate) Write(certPath, keyPath, csrPath string) error {
	...
	err := containerlabutils.CreateFile(certPath, string(c.Cert))
	if err != nil {
		return err
	}

	log.Debugf("writing key file to %s", keyPath)
	err = containerlabutils.CreateFile(keyPath, string(c.Key))
	if err != nil {
		return err
	}
	...
	if len(c.Csr) != 0 && csrPath != "" {
		...
		err = containerlabutils.CreateFile(csrPath, string(c.Csr))
		if err != nil {
			return err
		}
	}
