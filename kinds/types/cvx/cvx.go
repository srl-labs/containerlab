...
// AddToRegistry registers the node in the Registry.
func AddToRegistry(r *kinds.Registry) error {
	err := r.AddKnownKind(kindnames, func() nodes.Node {
		return new(cvx)
	}, nil)
	if err != nil {
		return err
	}
	return nodes.SetNonDefaultRuntimePerKind(kindnames, ignite.RuntimeName)
}
