...
But, of course, containerlab is perfectly capable of wiring up arbitrary containers which can be used as the test clients.
...
* Declarative way of defining the labs by means of [topology definition files](manual/topo-def-file.md).
* Focus on containerized NOSes. The configuration details of various NOS containers are abstracted to create a clean and lightweight structure.
* Automated TLS certificates provisioning. The nodes which require TLS certs will get them automatically on start.
* The "most-wanted" lab topologies are [documented and provided](lab-examples/lab-examples.md) with Containerlab installation.
