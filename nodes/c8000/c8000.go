...
func (n *c8000) CheckInterfaceName() error {
	...
	for _, e := range n.Config().Endpoints {
		if !ifRe.MatchString(e.Iface) {
			return fmt.Errorf("cisco 8000 interface name %q doesn't match the required pattern. Cisco 8000 interfaces should be named as Hu0_0_0_X (100G interfaces) or FH0_0_0_X (400G interfaces) where X is the interface number", e.Iface)
		}
	}
