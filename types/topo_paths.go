...
func (t *TopoPaths) SetLabDir(p string) (err error) {
	if !containerlabutils.DirExists(p) {
...
func (t *TopoPaths) SetExternalCaFiles(certFile, keyFile string) error {
	...
	certFile = containerlabutils.ResolvePath(certFile, t.TopologyFileDir())
	keyFile = containerlabutils.ResolvePath(keyFile, t.TopologyFileDir())

	if !containerlabutils.FileExists(certFile) {
		return fmt.Errorf("external CA cert file %s does not exist", certFile)
	}

	if !containerlabutils.FileExists(keyFile) {
...
func (*TopoPaths) ClabTmpDir() string {
	if !containerlabutils.DirExists(clabTmpDir) {
		containerlabutils.CreateDirectory(clabTmpDir, 0o755)
	}
...
func (t *TopoPaths) ClabBakDir() string {
	...
	if !containerlabutils.DirExists(d) {
		containerlabutils.CreateDirectory(d, 0o755)
	}
