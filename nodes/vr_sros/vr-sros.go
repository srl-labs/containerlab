...
func (s *vrSROS) Init(cfg *containerlabtypes.NodeConfig, opts ...containerlabnodes.NodeOption) error {
	// Init DefaultNode
	s.VRNode = *containerlabnodes.NewVRNode(s, defaultCredentials, scrapliPlatformName)
	// set virtualization requirement
	s.HostRequirements.VirtRequired = true
	s.LicensePolicy = containerlabtypes.LicensePolicyWarn
	// SR OS requires unbound pubkey authentication mode until this is
	// gets fixed in later SR OS release.
	s.SSHConfig.PubkeyAuthentication = containerlabtypes.PubkeyAuthValueUnbound
	...
	s.Cfg.Env = containerlabutils.MergeStringMaps(defEnv, s.Cfg.Env)
...
func (s *vrSROS) PreDeploy(_ context.Context, params *containerlabnodes.PreDeployParams) error {
...
func (s *vrSROS) PostDeploy(ctx context.Context, _ *containerlabnodes.PostDeployParams) error {
...
func (s *vrSROS) SaveConfig(_ context.Context) error {
	err := containerlabnetconf.SaveRunningConfig(s.Cfg.LongName,
...
func createVrSROSFiles(node containerlabnodes.Node) error {
	nodeCfg := node.Config()
	...
	if !isPartialConfigFile(nodeCfg.StartupConfig) {
		...
		if nodeConfigExists(nodeCfg.LabDir) {
			...
			containerlabnodes.LoadStartupConfigFileVr(node, configDirName, startupCfgFName)
		}
	}
...
func (s *vrSROS) isHealthy(ctx context.Context) bool {
	ex := containerlabexec.NewExecCmdFromSlice([]string{"grep", "0 running", "/health"})
...
func (s *vrSROS) applyPartialConfig(ctx context.Context, addr, platformName,
	...
	configContent, err := containerlabutils.SubstituteEnvsAndTemplate(config, s.Cfg)
	if err != nil {
		return err
	}
