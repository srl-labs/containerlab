...
func Test_vrSROS_mapSSHPubKeys(t *testing.T) {
	...
	keysSlice := make([]ssh.PublicKey, 0, len(keySlice))
	for _, strKey := range keySlice {
	...
	if len(tmplData.SSHPubKeysRSA) != expectedRSA {
		t.Errorf("expected %d RSA keys, got %d", expectedRSA, len(tmplData.SSHPubKeysRSA))
	}

	if len(tmplData.SSHPubKeysECDSA) != expectedECDSA {
		t.Errorf("expected %d ECDSA keys, got %d", expectedECDSA, len(tmplData.SSHPubKeysECDSA))
	}

}

func Test_vrSROS_mapSSHPubKeys_None(t *testing.T) {
	keySlice := [][]byte{}
