...
With `--runtime c9s`, `save` runs the inner `containerlab save` command inside each launcher pod. `save --copy` streams the resulting files back to the local destination using the same copy layout as Docker labs. See [Containerlab runtime](../manual/clabernetes/runtime.md#save) for details.
///
